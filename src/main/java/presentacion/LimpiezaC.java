...
public class LimpiezaC extends JDialog {
    private JTextField txtBuscar;
    private JScrollPane mainScrollPanel;
    private JTable tblLimp;
    private JButton btnVer;
    private JPanel mainPanel;
    private JButton btnRegresar;
    private MainForm mainForm;
    private ProductoDAO productoDAO;
    private final int ID_CATEGORIA = 4;
    ...
    public LimpiezaC(MainForm mainForm){
    ...
    private void hideCol(int pColumna) {
        this.tblLimp.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblLimp.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblLimp.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblLimp.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblLimp.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblLimp.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
