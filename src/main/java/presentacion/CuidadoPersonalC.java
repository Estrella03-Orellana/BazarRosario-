...
public class CuidadoPersonalC extends JDialog {
    private JTextField txtBuscar;
    private JScrollPane mainScrollPanel;
    private JTable tblCui;
    private JButton btnVer;
    private JPanel mainPanel;
    private JButton btnRegresar;
    private MainForm mainForm;
    private ProductoDAO productoDAO;
    private final int ID_CATEGORIA = 10;
    ...
    public CuidadoPersonalC(MainForm mainForm){
    ...
    private void hideCol(int pColumna) {
        this.tblCui.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblCui.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblCui.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblCui.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblCui.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblCui.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
