...
public class ConfiteriayPapeleriaC extends JDialog {
    ...
    private void hideCol(int pColumna) {
        this.tblConfi.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblConfi.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblConfi.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblConfi.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblConfi.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblConfi.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
