...
public class OfertasC extends JDialog {
    private JTextField txtBuscar;
    private JScrollPane mainScrollPanel;
    private JTable tblOfertas;
    ...
    public OfertasC(MainForm mainForm){
    ...
    private void getAll() {
        try {
            ...
            this.tblOfertas.setModel(model);
            Object row[] = null;
    ...
    private void hideCol(int pColumna) {
        this.tblOfertas.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblOfertas.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblOfertas.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblOfertas.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblOfertas.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblOfertas.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
