...
public class RopaC extends JDialog {
    private JTextField txtBuscar;
    private JScrollPane mainScrollPanel;
    private JTable tblRopa;
    ...
    public RopaC(MainForm mainForm){
    ...
    private void hideCol(int pColumna) {
        this.tblRopa.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblRopa.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblRopa.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblRopa.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblRopa.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblRopa.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
