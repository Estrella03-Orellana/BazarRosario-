...
public class JuguetesC extends JDialog {
    private JTextField txtBuscar;
    private JScrollPane mainScrollPanel;
    private JTable tblJug;
    private JButton btnVer;
    private JPanel mainPanel;
    private JButton btnRegresar;
    private MainForm mainForm;
    private ProductoDAO productoDAO;
    private final int ID_CATEGORIA = 3;
    ...
    public JuguetesC(MainForm mainForm){
    ...
    private void hideCol(int pColumna) {
        this.tblJug.getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblJug.getColumnModel().getColumn(pColumna).setMinWidth(0);
        this.tblJug.getTableHeader().getColumnModel().getColumn(pColumna).setMaxWidth(0);
        this.tblJug.getTableHeader().getColumnModel().getColumn(pColumna).setMinWidth(0);
    }

    private Producto getProductFromTableRow() {
        Producto producto = null;
        try {
            int filaSelect = this.tblJug.getSelectedRow();
            int id = 0;

            if (filaSelect != -1) {
                id = (int) this.tblJug.getValueAt(filaSelect, 0);
            } else {
                JOptionPane.showMessageDialog(null,
